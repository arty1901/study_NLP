...
namespace Pullenti.Ner.Geo.Internal
{
    ...
    {
        public static Pullenti.Ner.ReferentToken TryDefine(List<CityItemToken> li, Pullenti.Ner.Core.AnalyzerDataWithOntology ad, bool always = false)
        {
            if (li == null) 
                return null;
            ...
            if (res == null && always) 
            {
                if (OrgItemToken.TryParse(li[0].BeginToken) != null) 
                {
                }
                else 
            ...
            if ((res == null && li.Count == 1 && li[0].Typ == CityItemToken.ItemType.Noun) && li[0].GeoObjectBefore && (li[0].WhitespacesAfterCount < 3)) 
            {
                ...
                {
                    CityItemToken nam = CityItemToken.TryParse(rt.BeginToken, null, false);
                    if (nam != null && nam.EndToken == rt.EndToken && ((nam.Typ == CityItemToken.ItemType.ProperName || nam.Typ == CityItemToken.ItemType.City))) 
                    {
                        ...
                        return new Pullenti.Ner.ReferentToken(g, li[0].BeginToken, rt);
                    }
                }
            }
            if (res == null) 
        ...
        static Pullenti.Ner.ReferentToken Try1(List<CityItemToken> li, out Pullenti.Ner.Core.IntOntologyItem oi, Pullenti.Ner.Core.AnalyzerDataWithOntology ad)
        {
            ...
            {
                List<Pullenti.Ner.Address.Internal.StreetItemToken> sits = Pullenti.Ner.Address.Internal.StreetItemToken.TryParseList(li[0].BeginToken, 3);
            ...
            if (i < li.Count) 
            {
                if (li[i].Typ == CityItemToken.ItemType.Noun) 
                {
                    ...
                    {
                        Pullenti.Ner.Address.Internal.StreetItemToken sit = Pullenti.Ner.Address.Internal.StreetItemToken.TryParse(li[i].EndToken.Next, null, false);
                        if (sit != null && sit.Typ == Pullenti.Ner.Address.Internal.StreetItemType.Noun) 
                        {
                            at = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(li[i].BeginToken, false, null);
                            if (at != null) 
                            {
                                Pullenti.Ner.Address.Internal.AddressItemToken at2 = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(li[i].EndToken.Next, false, null);
            ...
            if (!ok) 
            {
                if (!Pullenti.Ner.Core.MiscHelper.IsExistsInDictionary(li[0].BeginToken, li[0].EndToken, Pullenti.Morph.MorphClass.Adjective | Pullenti.Morph.MorphClass.Noun | Pullenti.Morph.MorphClass.Pronoun)) 
                {
                    ...
                    if (ok && li[0].BeginToken == li[0].EndToken) 
                    {
                        ...
                        {
                            Pullenti.Ner.Address.Internal.AddressItemToken ad1 = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(li[0].BeginToken, false, null);
                            if (ad1 != null && ad1.Typ == Pullenti.Ner.Address.Internal.AddressItemType.Street) 
                            {
                                Pullenti.Ner.Address.Internal.AddressItemToken ad2 = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(li[0].EndToken.Next, false, null);
                                if (ad2 == null || ad2.Typ != Pullenti.Ner.Address.Internal.AddressItemType.Street) 
                                    ok = false;
                            }
                            else if (OrgItemToken.TryParse(li[0].BeginToken) != null) 
                                ok = false;
                        }
                    }
                }
        ...
        static Pullenti.Ner.ReferentToken _tryNounName(List<CityItemToken> li, out Pullenti.Ner.Core.IntOntologyItem oi, bool always)
        {
            ...
            if ((typ != null && li[i1].Typ == CityItemToken.ItemType.Noun && ((i1 + 1) < li.Count)) && li[0].WhitespacesAfterCount <= 1 && (((Pullenti.Morph.LanguageHelper.EndsWith(typ, "ПОСЕЛОК") || Pullenti.Morph.LanguageHelper.EndsWith(typ, "СЕЛИЩЕ") || typ == "ДЕРЕВНЯ") || typ == "СЕЛО"))) 
            {
                ...
                {
                    OrgItemToken ooo = OrgItemToken.TryParse(li[i1].BeginToken);
                    if (ooo != null) 
                        return null;
                }
                typ2 = li[i1].Value;
                ...
                if (!li[0].Doubtful) 
                {
                    ...
                    if (npt != null) 
                    {
                        if (npt.EndToken.EndChar > li[i1].EndChar && npt.Adjectives.Count > 0 && !npt.Adjectives[0].EndToken.Next.IsComma) 
                        {
                            ...
                            if (!ok) 
                            {
                                if (OrgItemToken.TryParse(li[i1].EndToken.Next) != null) 
                                    ok = true;
                            }
                        }
            ...
            if (!ok && !always) 
            {
                if (MiscLocationHelper.CheckNearBefore(li[0].BeginToken) == null) 
                    return null;
            }
            if (li.Count > (i1 + 1)) 
            ...
            {
                Pullenti.Ner.Address.Internal.AddressItemToken ait = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(li[1].BeginToken, false, null);
            ...
            else 
            {
                ...
                if (!ok) 
                {
                    for (Pullenti.Ner.Token tt2 = li[0].BeginToken.Previous; tt2 != null; tt2 = tt2.Previous) 
                    {
                        ...
                        if (ok) 
                        {
                            List<Pullenti.Ner.Address.Internal.StreetItemToken> sits = Pullenti.Ner.Address.Internal.StreetItemToken.TryParseList(li[0].BeginToken, 10);
        ...
        static Pullenti.Ner.ReferentToken Try4(List<CityItemToken> li)
        {
            if ((li.Count > 0 && li[0].Typ == CityItemToken.ItemType.Noun && ((li[0].Value != "ГОРОД" && li[0].Value != "МІСТО" && li[0].Value != "CITY"))) && ((!li[0].Doubtful || li[0].GeoObjectBefore))) 
            {
                ...
                {
                    OrgItemToken org = OrgItemToken.TryParse(li[0].EndToken.Next);
                    if (org != null) 
                    {
                        if (li.Count > 1 && OrgItemToken.TryParse(li[1].EndToken.Next) != null) 
                        {
                        }
                        else 
                        {
        ...
        public static bool CheckCityAfter(Pullenti.Ner.Token tt)
        {
            ...
            if (tt == null) 
                return false;
            List<CityItemToken> cits = CityItemToken.TryParseList(tt, 5);
            if (cits == null || cits.Count == 0) 
            {
                if (tt.LengthChar == 1 && tt.Chars.IsAllLower && ((tt.IsValue("Д", null) || tt.IsValue("П", null)))) 
                {
                    ...
                    if (tt1 != null && tt1.IsChar('.')) 
                        tt1 = tt1.Next;
                    CityItemToken ci = CityItemToken.TryParse(tt1, null, false);
