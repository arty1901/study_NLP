...
namespace Pullenti.Ner.Geo.Internal
{
    static class TerrDefineHelper
    {
        ...
        static Pullenti.Ner.ReferentToken _tryAttachPureTerr(List<TerrItemToken> li, Pullenti.Ner.Core.AnalyzerData ad)
        {
            ...
            if (org == null) 
                org = OrgItemToken.TryParse(tt);
            if (org != null) 
            {
                Pullenti.Ner.ReferentToken rt = CreateGeoOrgTerr(org);
                if (rt == null) 
                    return null;
            ...
            CityItemToken cit = CityItemToken.TryParse(tt, null, false);
            if (cit != null) 
            {
                bool ok = false;
                if (cit.Typ == CityItemToken.ItemType.ProperName) 
                    ok = true;
                else if (cit.Typ == CityItemToken.ItemType.City && tt != null && Pullenti.Ner.Core.BracketHelper.CanBeEndOfSequence(cit.EndToken.Next, false, null, false)) 
                    ok = true;
                if (ok && tt != t && !Pullenti.Ner.Core.BracketHelper.CanBeEndOfSequence(cit.EndToken.Next, false, null, false)) 
                    ok = false;
                if (ok) 
                {
                    if (tt == t) 
                    {
                        ok = false;
                        Pullenti.Ner.Address.Internal.AddressItemToken aa = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(t, false, null);
                        if (aa != null && aa.Typ == Pullenti.Ner.Address.Internal.AddressItemType.Street) 
                        {
                            Pullenti.Ner.Address.Internal.StreetItemToken aa0 = Pullenti.Ner.Address.Internal.StreetItemToken.TryParse(t, null, false);
                            if (aa0 != null && aa0.IsNumberKm) 
                                return null;
                            else 
                            {
                                Pullenti.Ner.Address.Internal.AddressItemToken aa2 = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(cit.EndToken.Next, false, null);
                                if (aa2 != null && aa2.Typ == Pullenti.Ner.Address.Internal.AddressItemType.Street) 
                                {
                                    ok = true;
                                    aa = null;
                                }
                        ...
                        if (aa != null && aa.Typ == Pullenti.Ner.Address.Internal.AddressItemType.Street) 
                        {
                            for (Pullenti.Ner.Token ttt = t; ttt != null && ttt.EndChar <= aa.EndChar; ttt = ttt.Next) 
                            {
                                if ((ttt is Pullenti.Ner.TextToken) && ttt.Chars.IsLetter && ttt.Chars.IsAllLower) 
                                    return null;
                            }
                            if (aa.EndChar > cit.EndChar) 
                                return null;
                        }
                        if (ok) 
                        {
                        }
                        else if (aa != null && aa.Typ == Pullenti.Ner.Address.Internal.AddressItemType.Street) 
                        {
                        }
                        else if (MiscLocationHelper.CheckGeoObjectBefore(li[0].BeginToken, false)) 
                        {
                        }
                        else if (MiscLocationHelper.CheckGeoObjectAfter(cit.EndToken, false)) 
                        {
                        }
                        else 
                            return null;
                    }
                    Pullenti.Ner.Geo.GeoReferent gg = new Pullenti.Ner.Geo.GeoReferent();
                    gg.AddTypTer(t.Kit.BaseLanguage);
                    gg.AddName(cit.Value);
                    Pullenti.Ner.ReferentToken rt = new Pullenti.Ner.ReferentToken(gg, li[0].BeginToken, cit.EndToken);
                    if (tt != t) 
        ...
        static Pullenti.Ner.ReferentToken CreateGeoOrgTerr(OrgItemToken org)
        {
            ...
            if (geo.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_TYPE, null, true) == null) 
                geo.AddTypTer(org.Kit.BaseLanguage);
            return new Pullenti.Ner.ReferentToken(geo, org.BeginToken, org.EndToken);
        }
        public static Pullenti.Ner.ReferentToken TryDefine(List<TerrItemToken> li, Pullenti.Ner.Core.AnalyzerData ad, bool attachAlways = false, List<CityItemToken> cits = null, List<Pullenti.Ner.Geo.GeoReferent> exists = null)
        {
            ...
            for (k = 0; k < li.Count; k++) 
            {
                    ...
                    if (noun != null) 
                    {
                        if (k == 1) 
                        {
                            if (noun.TerminItem.CanonicText == "РАЙОН" || noun.TerminItem.CanonicText == "ОБЛАСТЬ" || noun.TerminItem.CanonicText == "СОЮЗ") 
                            {
                                ...
                                if (!ok) 
                                {
                                    Pullenti.Ner.Address.Internal.AddressItemToken adr = Pullenti.Ner.Address.Internal.AddressItemToken.TryParse(tt, false, null);
                                    if (adr != null) 
                                    {
                ...
                else if (li[k].TerminItem != null) 
                {
                    ...
                    else 
                    {
                        ...
                        if (k == 0) 
                        {
                            TerrItemToken tt = TerrItemToken.TryParse(li[k].BeginToken.Previous, null);
            ...
            if (exObj != null) 
            {
                if (exObj.IsAdjective && !exObj.Morph.Language.IsEn && noun == null) 
                {
                    if (attachAlways && exObj.EndToken.Next != null) 
                    {
                        ...
                        {
                            Pullenti.Ner.Address.Internal.StreetItemToken str = Pullenti.Ner.Address.Internal.StreetItemToken.TryParse(exObj.EndToken.Next, null, false);
                            if (str != null) 
                            {
                    ...
                    {
                        CityItemToken cit = CityItemToken.TryParse(exObj.EndToken.Next, null, false);
                ...
                if (newName.IsAdjective) 
                {
                        ...
                        if (Pullenti.Ner.Core.MiscHelper.IsExistsInDictionary(newName.BeginToken, newName.EndToken, Pullenti.Morph.MorphClass.Adjective | Pullenti.Morph.MorphClass.Pronoun | Pullenti.Morph.MorphClass.Verb)) 
                        {
                            if (noun.BeginToken != newName.BeginToken) 
                            {
                                if (geoBefore == null) 
                                {
                                    ...
                                    else if (OrgItemToken.TryParse(newName.EndToken.Next) != null) 
                                    {
                                    }
                                    else 
                                        return null;
                    ...
                    else 
                    {
                        ...
                        else if (k == li.Count && !newName.IsAdjInDictionary) 
                        {
                            ...
                            {
                                CityItemToken cit1 = CityItemToken.TryParseBack(li[0].BeginToken.Previous);
                        ...
                        {
                            CityItemToken cit = CityItemToken.TryParse(li[2].BeginToken, null, false);
                            if (cit != null) 
                            {
                ...
                else 
                {
                    ...
                    if (newName.BeginToken != newName.EndToken) 
                    {
                        for (Pullenti.Ner.Token ttt = newName.BeginToken.Next; ttt != null && ttt.EndChar <= newName.EndChar; ttt = ttt.Next) 
                        {
                            ...
                            {
                                TerrItemToken ty = TerrItemToken.TryParse(ttt, null);
            ...
            else 
            {
                ...
                if ((li.Count == 1 && noun == li[0] && li[0].TerminItem != null) && TerrItemToken.TryParse(li[0].EndToken.Next, null) == null && TerrItemToken.TryParse(li[0].BeginToken.Previous, null) == null) 
                {
                    ...
                    for (Pullenti.Ner.Token tt = li[0].BeginToken.Previous; tt != null; tt = tt.Previous) 
                    {
                        ...
                        for (tt = li[0].EndToken.Next; tt != null; tt = tt.Next) 
                        {
                            ...
                            if (cou > 500) 
                                break;
                            TerrItemToken tee = TerrItemToken.TryParse(tt, null);
                            if (tee == null) 
                                continue;
                            ok = false;
                            break;
        ...
        static bool _canBeGeoAfter(Pullenti.Ner.Token tt)
        {
            ...
            if (tt.GetReferent() is Pullenti.Ner.Geo.GeoReferent) 
                return true;
            List<TerrItemToken> tli = TerrItemToken.TryParseList(tt, 2);
