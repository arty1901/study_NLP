...
namespace Pullenti.Ner.Geo.Internal
{
    public static class MiscLocationHelper
    {
        public static bool CheckGeoObjectBefore(Pullenti.Ner.Token t, bool pureGeo = false)
        {
            ...
            {
                CityItemToken cit2 = CityItemToken.TryParse(t.Previous, null, false);
                if (cit2 != null && cit2.Typ != CityItemToken.ItemType.Noun && cit2.EndToken.Next == t) 
                {
                    CityItemToken cit1 = CityItemToken.TryParse(t.Previous.Previous, null, false);
                    ...
                    if (cit1 == null && t.Previous.Previous.IsChar('.') && t.Previous.Previous.Previous != null) 
                    {
                        ...
                        cit1 = CityItemToken.TryParse(tt, null, false);
                        if (cit1 != null && cit1.Typ == CityItemToken.ItemType.Noun) 
                            return true;
        ...
        public static bool CheckGeoObjectAfter(Pullenti.Ner.Token t, bool dontCheckCity = false)
        {
            ...
            for (Pullenti.Ner.Token tt = t.Next; tt != null; tt = tt.Next) 
            {
                ...
                if (tt.Morph.Class.IsPreposition) 
                {
                    if (!dontCheckCity && tt.IsValue("С", null) && tt.Next != null) 
                    {
                        ...
                        if (ttt.IsChar('.') && (ttt.WhitespacesAfterCount < 3)) 
                            ttt = ttt.Next;
                        List<CityItemToken> cits = CityItemToken.TryParseList(ttt, 3);
                ...
                if (rt == null) 
                {
                    if (!dontCheckCity) 
                    {
                        List<CityItemToken> cits = CityItemToken.TryParseList(tt, 3);
                        ...
                        {
                            if (OrgItemToken.TryParse(cits[0].EndToken.Next) != null) 
                                return true;
                        }
                    }
