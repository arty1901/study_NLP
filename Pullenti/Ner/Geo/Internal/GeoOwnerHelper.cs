            ...
            if (hi.IsCity) 
            {
                ...
                if (lo.IsCity) 
                {
                    ...
                    if (hit.Contains("город;") || hit.Contains("місто") || hit.Contains("city")) 
                    {
                        ...
                        if (((lot.Contains("поселок") || lot.Contains("селище") || lot.Contains("хутор")) || lot.Contains("село") || lot.Contains("деревня")) || lot.Contains("городок") || lot.Contains("местечко")) 
                            return true;
                    }
                    if (hit.Contains("поселение") || hit.Contains("поселок")) 
                    {
                        if (lot.Contains("деревня") && hit.Contains("коттеджный поселок")) 
                            return false;
                        if ((((lot.Contains("село;") || lot.Contains("деревня") || lot.Contains("хутор")) || lot.Contains("аул") || lot.Contains("станица")) || lot.Contains("починок") || lot.Contains("заимка")) || lot.Contains("местечко")) 
                            return true;
                    }
                    if (hit.Contains("деревня")) 
                    {
                        if (lot.Contains("населенный пункт") || lot.Contains("коттеджный поселок")) 
                            return true;
                    }
