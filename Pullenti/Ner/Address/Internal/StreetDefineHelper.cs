...
namespace Pullenti.Ner.Address.Internal
{
    public static class StreetDefineHelper
    {
        public static bool CheckStreetAfter(Pullenti.Ner.Token t)
        {
            ...
            List<StreetItemToken> li = StreetItemToken.TryParseList(t, 10);
            if (li == null) 
                return false;
        ...
        internal static AddressItemToken TryParseStreet(List<StreetItemToken> sli, bool extOntoRegim = false, bool forMetro = false)
        {
            ...
            if (before > after) 
            {
                if (noun.Termin.CanonicText == "МЕТРО") 
                    return null;
                if (noun.Termin.CanonicText == "КВАРТАЛ") 
                {
            ...
            if (t1 != null && t1.IsComma) 
                t1 = t1.Next;
            StreetItemToken non = StreetItemToken.TryParse(t1, null, false);
            ...
            if (street.FindSlot(Pullenti.Ner.Address.StreetReferent.ATTR_NAME, "ПРОЕКТИРУЕМЫЙ", true) != null && street.Number == null) 
            {
                ...
                else 
                {
                    ...
                    if (ttt != null) 
                    {
                        non = StreetItemToken.TryParse(ttt, null, false);
                        if (non != null && non.Typ == StreetItemType.Number) 
                        {
                            street.Number = non.Number.Value;
                            res.EndToken = non.EndToken;
            ...
            {
                AddressItemToken ait = AddressItemToken.TryParsePureItem(res.EndToken.Next, null);
        ...
        static AddressItemToken TryDetectNonNoun(List<StreetItemToken> sli, bool ontoRegim, bool forMetro)
        {
            ...
            if (sli.Count == 1 && sli[0].Typ == StreetItemType.Name && ((ontoRegim || forMetro))) 
            {
                ...
                if (!forMetro && !sli[0].IsInDictionary && sli[0].ExistStreet == null) 
                {
                    ...
                    if (tt != null && tt.IsComma) 
                        tt = tt.Next;
                    AddressItemToken ait1 = AddressItemToken.TryParse(tt, true, null);
                ...
                if (!ontoRegim) 
                {
                    ...
                    for (tt = sli[0].EndToken.Next; tt != null; tt = tt.Next) 
                    {
                        if (!tt.IsCommaAnd || tt.Next == null) 
                            break;
                        List<StreetItemToken> sli2 = StreetItemToken.TryParseList(tt.Next, 10);
                        if (sli2 == null) 
                            break;
            ...
            {
                AddressItemToken ait = AddressItemToken.TryParse(t, false, null);
                if (ait == null) 
                    return null;
                ...
                else if (ait.Typ == AddressItemType.Number && (ait.BeginToken.WhitespacesBeforeCount < 2)) 
                {
                    ...
                    for (t = sli[0].BeginToken.Previous; t != null; t = t.Previous) 
                    {
                        ...
                        if (t.IsChar('.')) 
                            break;
                        AddressItemToken ait0 = AddressItemToken.TryParsePureItem(t, null);
                        if (ait != null) 
                        {
            ...
            if (!ok) 
                return null;
            Pullenti.Ner.Geo.Internal.OrgItemToken ooo = Pullenti.Ner.Geo.Internal.OrgItemToken.TryParse(sli[0].BeginToken);
            if (ooo == null && sli.Count > 1) 
                ooo = Pullenti.Ner.Geo.Internal.OrgItemToken.TryParse(sli[1].BeginToken);
            if (ooo != null) 
                return null;
            street = new Pullenti.Ner.Address.StreetReferent();
        ...
        {
            List<StreetItemToken> sli = StreetItemToken.TryParseList(t1, 10);
            if (sli == null || (sli.Count < 1) || sli[0].Typ != StreetItemType.Noun) 
                return null;
            List<StreetItemToken> sli2 = StreetItemToken.TryParseList(t2, 10);
